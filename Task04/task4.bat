...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT COUNT(*) AS 'Количество оценок', AVG(r.rating) AS 'Средняя оценка' FROM ratings r JOIN users u ON r.user_id = u.id WHERE u.gender = 'male' AND strftime('%Y', datetime(r.timestamp, 'unixepoch')) BETWEEN '2011' AND '2014'"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "SELECT m.title AS 'Название фильма', m.year AS 'Год выпуска', AVG(r.rating) AS 'Средняя оценка', COUNT(DISTINCT r.user_id) AS 'Количество пользователей' FROM movies m JOIN ratings r ON m.id = r.movie_id GROUP BY m.id, m.title, m.year ORDER BY m.year, m.title LIMIT 20"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH RECURSIVE split(genre, rest) AS (SELECT '', genres || '|' FROM movies WHERE genres != '(no genres listed)' UNION ALL SELECT substr(rest, 0, instr(rest, '|')), substr(rest, instr(rest, '|')+1) FROM split WHERE rest != ''), genre_counts AS (SELECT trim(genre) AS genre, COUNT(*) AS count FROM split WHERE genre != '' GROUP BY trim(genre)) SELECT genre AS 'Жанр', count AS 'Количество фильмов' FROM genre_counts ORDER BY count DESC LIMIT 1"
echo " "
...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "WITH RECURSIVE years(year) AS (VALUES(2000) UNION ALL SELECT year + 1 FROM years WHERE year < 2024) SELECT year AS 'Год', CASE strftime('%w', year || '-07-27') WHEN '0' THEN 'Воскресенье' WHEN '1' THEN 'Понедельник' WHEN '2' THEN 'Вторник' WHEN '3' THEN 'Среда' WHEN '4' THEN 'Четверг' WHEN '5' THEN 'Пятница' WHEN '6' THEN 'Суббота' END AS 'День недели' FROM years ORDER BY year"
echo " "